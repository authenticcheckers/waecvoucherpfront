...
export default function SuccessPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center font-bold">Validating...</div>}>
      <SuccessContent />
    </Suspense>
  );
}
...
function SuccessContent() {
  ...
  const downloadPDF = async (v: any) => {
    ...
    link.download = `WAEC-Voucher.pdf`;
    link.click();
  };
  ...
  return (
    <main className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-xl w-full bg-white rounded-[3rem] shadow-2xl overflow-hidden">
        ...
        <div className="p-10">
          {loading ? <p className="text-center font-bold">Fetching codes...</p> : (
            <div className="space-y-6">
              {vouchers.map((v) => (
                <div key={v.serial} className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl p-6 text-center">
                  <p className="text-2xl font-black text-amber-600 mb-1">PIN: {v.pin}</p>
                  <p className="text-sm font-bold text-slate-400 mb-4">Serial: {v.serial}</p>
                  <button onClick={() => downloadPDF(v)} className="inline-flex items-center gap-2 text-xs font-black text-slate-800 bg-white border border-slate-200 px-4 py-2 rounded-xl">
          ...
          <div className="mt-12 text-center">
            <h3 className="font-black text-slate-900 mb-2">Help a Classmate!</h3>
            <p className="text-slate-500 text-sm mb-8">Share this with your friends who need vouchers.</p>
            <button 
              ...
              className="w-full bg-[#25D366] text-white py-5 rounded-2xl font-black flex items-center justify-center gap-3 shadow-xl shadow-green-200"
            >
              <MessageCircle size={22} /> Share on WhatsApp
            </button>
          </div>
        </div>
      </motion.div>
      <Link href="/" className="mt-10 font-black text-slate-400 hover:text-slate-900">Return Home</Link>
    </main>
  );
}
